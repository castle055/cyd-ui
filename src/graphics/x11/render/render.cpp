...
void render_sbr(cydui::graphics::window_t* win) {
  ...
  while (!win->render_reqs.empty()) {
    ...
    if (req.x < minx) minx = req.x;
    if (req.y < miny) miny = req.y;
    if (req.x + req.w > maxx) maxx = req.x + req.w;
    if (req.y + req.h > maxy) maxy = req.y + req.h;
...
//    XClearArea(state::get_dpy(), win->xwin, req.x, req.y, req.w, req.h, False);
  ...
  win->x_mtx.lock();
  XCopyArea(state::get_dpy(), win->drawable, win->xwin, win->gc, minx, miny, maxx-minx, maxy-miny, minx, miny);
  win->x_mtx.unlock();
...
void render::start(cydui::graphics::window_t* win) {
  if (win->render_thd && win->render_thd->running) return;
...
static void req(cydui::graphics::window_t* win, int x, int y, int w, int h) {
  ...
  xlog_ctrl.debug("REQ Render area, pos(x=%d,y=%d) size(w=%d,h=%d)", req.x, req.y, req.w, req.h);
  win->render_mtx.lock();
...
void render::clr_rect(cydui::graphics::window_t *win, int x, int y, unsigned int w, unsigned int h) {
...
void render::drw_line(cydui::graphics::window_t *win, cydui::layout::color::Color* color, int x, int y, int x1, int y1) {
  if (color) {
...
void render::drw_rect(cydui::graphics::window_t *win, cydui::layout::color::Color* color, int x, int y, int w, int h, bool filled) {
  if (color) {
    xlog_ctrl.error("color: %s", color->to_string().c_str());
    XSetForeground(state::get_dpy(), win->gc, color_to_xcolor(color));
  }
...
void render::drw_arc(cydui::graphics::window_t *win, cydui::layout::color::Color* color, int x, int y, int w, int h, int a0, int a1, bool filled) {
  win->x_mtx.lock();
  if (color) {
    xlog_ctrl.error("color: %s", color->to_string().c_str());
    XSetForeground(state::get_dpy(), win->gc, color_to_xcolor(color));
