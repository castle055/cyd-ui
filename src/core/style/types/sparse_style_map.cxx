...
module cydui.styling.sparse_style_map;

import std;
import reflect;
import fabric.logging;

import cydui.styling.style_base;
import cydui.styling.sparse_field_map;

using namespace cydui::style;

sparse_style_map::sparse_style_map(const refl::type_info& type)
    : base_fields_(refl::type_info::from<style_base_t>()),
      impl_fields_(type) {}

sparse_style_map::sparse_style_map(const sparse_style_map& other)
    : base_fields_(other.base_fields_),
      impl_fields_(other.impl_fields_) {}

sparse_style_map& sparse_style_map::operator=(const sparse_style_map& other) {
...
sparse_style_map& sparse_style_map::operator=(sparse_style_map&& other) {
  if (other.get_type().id() != this->get_type().id()) {
    LOG::print{ERROR
    }("Assigning style map of type {} to map of type {}",
      other.get_type().name(),
...
bool sparse_style_map::operator==(const sparse_style_map& other) const {
  return base_fields_ == other.base_fields_ and impl_fields_ == other.impl_fields_;
}

const refl::type_info& sparse_style_map::get_type() const {
  return impl_fields_.get_type();
}

void sparse_style_map::apply_to_object(style_object_t& obj) {
  apply_map(base_fields_, &obj.get_base());
  apply_map(impl_fields_, obj.get_impl());
}

void sparse_style_map::clear() {
  base_fields_.clear();
  impl_fields_.clear();
}

std::optional<refl::field_path> sparse_style_map::find_path(const std::string& path_str) const {
  return base_fields_.find_path(path_str).or_else([&] { return impl_fields_.find_path(path_str); });
}

bool sparse_style_map::erase_path(const refl::field_path& path) {
  bool changed = false;
...
bool sparse_style_map::set_field(
...
std::optional<refl::any_ref> sparse_style_map::get_field(const refl::field_path& path) {
  if (path.root_type().id() == refl::type_id<style_base_t>) {
...
std::optional<refl::any> sparse_style_map::get_field(const refl::field_path& path) const {
  if (path.root_type().id() == refl::type_id<style_base_t>) {
    return base_fields_.get(path);
  } else {
    return impl_fields_.get(path);
...
std::optional<refl::any_ref> sparse_style_map::get_field(const std::string& path_str) {
  std::optional<refl::field_path> path_opt = find_path(path_str);
  if (not path_opt.has_value()) {
    LOG::print{WARN
    }("Unknown style field path: ({}|{})::{}",
...
std::optional<refl::any> sparse_style_map::get_field(const std::string& path_str) const {
  std::optional<refl::field_path> path_opt = find_path(path_str);
  if (not path_opt.has_value()) {
    LOG::print{WARN
    }("Unknown style field path: ({}|{})::{}",
...
bool sparse_style_map::empty() const {
  return base_fields_.empty() and impl_fields_.empty();
}

std::size_t sparse_style_map::size() const {
  return base_fields_.size() + impl_fields_.size();
}
void sparse_style_map::apply_map(
...
bool sparse_style_map::set_field_as_is(
  const refl::field_path& path,
  const refl::any&        value
) {
  bool changed = false;
