...
containers::VBox::VBox(cydui::components::ComponentState* state,
    bool border,
    int spacing,
...
void containers::VBox::on_redraw(cydui::events::layout::CLayoutEvent* ev) {
  if (border) {
    ...
    add({new primitives::Rectangle(
        c, state->x, state->y, state->w - 1, state->h - 1, false)});
  }
...
containers::HBox::HBox(cydui::components::ComponentState* state,
    bool border,
    int spacing,
    std::vector<cydui::components::Component*> children)
    : Component(state, children) {
