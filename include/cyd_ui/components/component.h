...
namespace cyd::ui::components {
  struct component_state_t {
    ...
    std::optional<std::shared_ptr<component_state_t>> parent = std::nullopt;
    std::unordered_map<std::string, std::shared_ptr<component_state_t>> children_states{};
    ...
    template<cyd::fabric::async::EventType T>
  ...
  struct component_base_t {
    ...
    std::vector<cyd::fabric::async::listener_t*> subscribed_listeners{};
  ...
  protected:
    void add_event_listeners(const std::unordered_map<std::string, event_handler_t::listener_data_t> &listeners) {
      this->state.transform(
        [&](std::shared_ptr<component_state_t> &s) {
    ...
    void add_children(
      ...
      for (auto &item: children_to_add) {
        for (auto &component_pair: item.get_components()) {
          ...
          std::shared_ptr<component_state_t> child_state;
  ...
  public:
    ...
    void configure_event_handler() override {
      ...
      evh->state = std::dynamic_pointer_cast<typename T::state_t>(state.value());
      evh->window = (std::dynamic_pointer_cast<typename T::state_t>(state.value()))->window;
      evh->props = &(static_cast<T*>(this)->props);
      evh->attrs = (attrs_component<T>*) this;
      evh->get_dim = [this] {
        return get_dimensional_relations();
      };
      evh->$children = [this] {
        return children;
      };
    ...
    get_fragment(
      ...
      compositing_node->id = (unsigned long) (this->state.value().get());
      compositing_node->op = {
        .x = this->_x.val(),
        .y = this->_y.val(),
