...
namespace cydui {
  ...
  //   bind(F T::* field) {}
  //
  //   bind& transform(std::function<F(const F&)>) {
  //     return *this;
  //   }
  // };
  ...
  private:
    // This object will own the context if it couldn't be found and thus a default one was created
    components::component_state_ref state = nullptr;
  ...
  struct use_context_delegate {
    ...
    static void
    set_state(use_context<ContextType>* it, const cydui::components::component_state_ref& state) {
      it->state = state;
    }
    template <typename ContextType>
  ...
  struct provide_context_delegate {
    template <typename ContextType>
    static void
    set_state(provide_context<ContextType>* it, const components::component_state_ref& state) {
      it->state = state;
    }
    template <typename ContextType>
    static void
    set_context(provide_context<ContextType>* it, const std::shared_ptr<ContextType>& ctx) {
      it->context_ = ctx;
    }
  };
}
