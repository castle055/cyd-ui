...
export module cydui.components.blueprint.event_dispatcher;
...
export import cydui.components.blueprint.event_handler;
export import cydui.components.blueprint.custom_event_listener;
export import cydui.components.contexts.api;
...
namespace cydui::components {
  ...
  public:
    ...
    void dispatch_mount(const content_type& content_children_builder) override {
      ...
      eh->on_mount(
        geom.position[geometry::X_AXIS],
        geom.position[geometry::Y_AXIS],
        geom.size[geometry::X_AXIS],
        geom.size[geometry::Y_AXIS],
    ...
    content_type update(
      ...
      return eh->on_redraw(
        geom.position[geometry::X_AXIS],
        geom.position[geometry::Y_AXIS],
        geom.viewport_size[geometry::X_AXIS],
        geom.viewport_size[geometry::Y_AXIS],
        content_children_builder
      );
    }
    void paint_fragment(vg::fragment_t& fragment) override {
      ...
      eh->draw_fragment(
        fragment,
        dimensions::get_value(geom.position[geometry::X_AXIS]),
        dimensions::get_value(geom.position[geometry::Y_AXIS]),
        dimensions::get_value(geom.viewport_size[geometry::X_AXIS]),
        dimensions::get_value(geom.viewport_size[geometry::Y_AXIS])
      );
    }
