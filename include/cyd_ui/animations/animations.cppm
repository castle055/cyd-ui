...
namespace cyd::ui {
  ...
  public:
    ...
    explicit animation(std::initializer_list<keyframe> keyframes, animation_opts opts = {})
        : data_(std::shared_ptr<animation_data>(new animation_data{keyframes, opts})) {}
  ...
  public:
    ...
    void run() override {
      ...
      std::forward_list<components::component_base_t::sptr> c_pending_compose{};
      ...
      // Repaint components that need it
      if (not c_pending_repaint.empty()) {
        for (const auto& c: c_pending_repaint) {
          s_repaint.emit(c);
        }
        s_render_all.emit(needs_compositing);
      }
  ...
  export void animate(const components::component_base_t::sptr& component, animation& anim) {
    auto c_state = component->state();
    if (c_state != nullptr) {
