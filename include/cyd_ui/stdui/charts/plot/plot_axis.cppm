...
namespace charts {
  ...
  export COMPONENT(PlotAxis, {
    ...
    ON_REDRAW {
      ...
      for (int step_i = 0; step_i < num_steps; step_i++) {
        ...
        if (props->axis_direction[0] > 0) {
          label_dim.x = $cw() * pos[0] - (label_dim.w / 2) + (props->label_direction[0] * props->label_offset);
        } else if (props->axis_direction[0] < 0) {
          label_dim.x = $cw() * pos[0] - (label_dim.w / 2) + (props->label_direction[0] * props->label_offset) + $cw();
        } else {
        ...
        if (props->axis_direction[1] > 0) {
          label_dim.y = $ch() * pos[1] + (label_dim.h / 2) + (props->label_direction[1] * props->label_offset);
        } else if (props->axis_direction[1] < 0) {
          label_dim.y = $ch() * pos[1] + (label_dim.h / 2) + (props->label_direction[1] * props->label_offset) + $ch();
        } else {
    ...
    FRAGMENT {
      ...
      double x1 = 0;
      double y1 = 0;
      double x2 = props->axis_direction[0] * ($ch().val() / props->axis_direction[1]);
      double y2 = props->axis_direction[1] * ($cw().val() / props->axis_direction[0]);
      if (y2 > $ch().val()) {
        y2 = $ch().val();
      } else if (y2 < -$ch().val()) {
        y2 = -$ch().val();
        ...
        fragment.append(vg::line { }
                        .x1(x1).y1(y1).x2($cw()).y2($ch())
                        .stroke_width(2).stroke("#FFFFFF"_color));
      }
      fragment.append(vg::line { }
                      .x1(x1 + offset_x).y1(y1 + offset_y).x2(x2 + offset_x).y2(y2 + offset_y)
                      .stroke_width(2).stroke("#777777"_color));
      const auto line_length = std::sqrt(la::vec<2> {x2 - x1, y2 - y1}.mag2());
      ...
      if (props->major_ticks_show) {
        for (la::scalar X = start; X < props->max; X += props->step) {
          ...
          x1 = line_length * pos[0] + offset_x;
          y1 = line_length * pos[1] + offset_y;
          if (X >= props->min && X <= props->max) {
            fragment.append(vg::line { }
                            .x1(x1).y1(y1)
                            .x2(x1 + props->label_direction[0] * props->label_offset * 0.25)
                            .y2(y1 + props->label_direction[1] * props->label_offset * 0.25)
          ...
          if (props->minor_ticks_show) {
            ...
            for (la::scalar Y = minor_step; Y <= props->step - minor_step && (X + Y) < props->max; Y += minor_step) {
              ...
              if ((X + Y) >= props->min && (X + Y) <= props->max) {
                fragment.append(vg::line { }
                                .x1(x1).y1(y1)
                                .x2(x1 + props->label_direction[0] * props->label_offset * 0.15)
                                .y2(y1 + props->label_direction[1] * props->label_offset * 0.15)
                                // .stroke_width(1).stroke("#555555"_color));
                                .stroke_width(1).stroke("#777777"_color));
              }
            }
          }
        }
      ...
      if (!props->title.empty()) {
        auto title = vg::text {props->title}
        ...
        auto pos   = (props->axis_direction * ratio * line_length) + (props->label_direction * props->title_offset);
        auto angle = std::atan2(props->axis_direction[1], props->axis_direction[0]) * 180.0 / std::numbers::pi;

        title.x(pos[0] + offset_x - text_offset_x);
        title.y(pos[1] + offset_y + text_offset_y);
        title.rotate(angle);
  ...
  export COMPONENT(NumericAxisLabel, {
    ...
    ON_REDRAW {
      ...
      (void)$w(label_footprint.w);
      (void)$h(label_footprint.h);
      return { };
    }
  ...
  struct axis_t {
    ...
    C& label_component(const std::function<cyd::ui::components::component_holder_t(la::scalar)>& builder) {
      label_builder_ = [&](const la::scalar value) {
  ...
  private:
    ...
    PlotAxis build_component(const auto& x, const auto& y, const auto& w, const auto& h) {
      PlotAxis pa {
        {
          ...
          .move_callback = [&](const la::scalar dx) {
            min_value_ += dx;
            max_value_ += dx;
          },
          .reset_callback = [&] {
