...
namespace charts {
    ...
    FRAGMENT {
      if (props.major_ticks_x_show) {
        ...
        for (la::scalar X = start; X < props.max_x; X += props.step_x) {
          ...
          x1 = $width.value_as_base_unit() * pos_x;
          if (X >= props.min_x && X <= props.max_x) {
            fragment.append(
              vg::line{}.x1(x1).y1(0).x2(x1).y2($height).stroke_width(2).stroke("#555555"_color)
            );
          }
          ...
          if (props.minor_ticks_x_show) {
            ...
            for (la::scalar Y = minor_step; Y <= props.step_x - minor_step && (X + Y) < props.max_x;
              ...
              if ((X + Y) >= props.min_x && (X + Y) <= props.max_x) {
                fragment.append(
                  vg::line{}.x1(x1).y1(0).x2(x1).y2($height).stroke_width(1).stroke("#555555"_color)
                );
              }
            }
          }
      ...
      if (props.major_ticks_y_show) {
        ...
        for (la::scalar X = start; X < props.max_y; X += props.step_y) {
          ...
          y1 = $height.value_as_base_unit() - $height.value_as_base_unit() * pos_y;
          if (X >= props.min_y && X <= props.max_y) {
            fragment.append(
              vg::line{}.x1(0).y1(y1).x2($width).y2(y1).stroke_width(2).stroke("#555555"_color)
            );
          }
          ...
          if (props.minor_ticks_y_show) {
            ...
            for (la::scalar Y = minor_step; Y <= props.step_y - minor_step && (X + Y) < props.max_y;
              ...
              if ((X + Y) >= props.min_y && (X + Y) <= props.max_y) {
                fragment.append(
                  vg::line{}.x1(0).y1(y1).x2($width).y2(y1).stroke_width(1).stroke("#555555"_color)
                );
              }
            }
          }
  ...
  struct grid_t {
    ...
    struct axis_ticks {
      explicit axis_ticks(grid_t& grid)
          : ref_(&grid) {}
