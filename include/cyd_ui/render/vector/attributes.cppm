...
export namespace vg {
  namespace paint {
    ...
    struct base_i {
      ...
      virtual void         apply_to_source(cydui::graphics::pixelmap_editor_t& editor, double opacity) const = 0;
      virtual cydui::Color sample(int x, int y) const                                       = 0;
    ...
    struct solid: public base_i {
      cydui::Color c = "#00000000"_color;

      solid() = default;
      solid(cydui::Color _c)
          : c(_c) {}

      void apply_to_source(cydui::graphics::pixelmap_editor_t& editor, double opacity) const override {
        editor->set_source_rgba(c.r, c.g, c.b, c.a * opacity);
      }

      cydui::Color sample(int x, int y) const override {
        return c;
      }
    ...
    namespace gradient {
      ...
      struct linear: public gradient::base_i {
        linear(
          ...
          const std::vector<std::pair<double, cydui::Color>>& _color_stops
        ) {
          x0 = start.first;
          y0 = start.second;
          x1 = end.first;
...
#define VG_ATTRIBUTE_DIMENSION(NAME, DEFAULT)                                                      \
  ...
  protected:
    void apply_stroke(cydui::graphics::pixelmap_editor_t& editor) const {
    ...
    void set_source_to_stroke(cydui::graphics::pixelmap_editor_t& editor) const {
      this->_stroke.paint_data->apply_to_source(editor, this->_stroke_opacity);
    }

    [[nodiscard]]
    cydui::Color sample_stroke(int x, int y) const {
  ...
  protected:
    ...
    [[nodiscard]]
    cydui::Color sample_fill(int x, int y) const {
  ...
  struct attrs_font: attr_font_family<T>,
                     ...
                     attr_font_weight<T> {
  protected:
    void apply_font(cydui::graphics::pixelmap_editor_t& editor) const {
