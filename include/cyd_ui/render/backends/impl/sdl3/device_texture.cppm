...
export namespace cydui::backends {
  struct device_texture_t: public backends::texture_base {
    ...
    void update_with(const graphics::pixelmap_t& pm) {
      if (texture == nullptr)
        return;
      ...
      // } else {
      SDL_UpdateTexture(texture, nullptr, pm.data, pm.width() * sizeof(graphics::pixel_t));
      // }
    }
  ...
  private:
    ...
    bool                           locked_    = false;
    std::vector<graphics::pixel_t> pixels_{};
    int                            pitch_ = 0;
  };
