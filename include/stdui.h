        ...
        COMPONENT(Flow, ;
          ...
          void horizontal_fixed_width() {
            ...
            for (auto &child: chi) {
              ...
              ats->h($ch());
              ats->x(cydui::dimensions::dimensional_relation_t {[=, this]() {
                int total_w = 0;
                for (const auto &dim: deps) {
                  if (dim != &attrs->_w) { // avoid summing the container's width
                    total_w += dim->val();
                  }
                }
          ...
          void vertical_fixed_height() {
            ...
            for (auto &child: chi) {
              auto ats = child->attrs();
              ats->w($cw());
              ats->y(cydui::dimensions::dimensional_relation_t {[=, this]() {
                int total_h = 0;
                for (const auto &dim: deps) {
        ...
        COMPONENT(Grid, {
          ...
          ON_REDRAW {
            ...
            for (auto &child: chi) {
              ...
              ats->w(cydui::dimensions::dimensional_relation_t {[=, this]() {
                auto [grid_w, _] = state->sizes[child];
                int total_w = attrs->_w;
                int step = total_w / (int) props->cols;
                return grid_w * step - props->x_gap;
              }, deps});
              ats->h(cydui::dimensions::dimensional_relation_t {[=, this]() {
